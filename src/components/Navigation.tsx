...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Theme Toggle Button (Discreet in Side Bar) */}
      <div className="mt-auto pt-3 border-t border-slate-800/80">
        <button
          ...
          className={`w-full flex items-center rounded-xl text-xs font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800/80 transition-all duration-200 cursor-pointer group ${
            isCollapsed ? 'justify-center p-3' : 'justify-between px-3 py-2'
          }`}
        >
          <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5'}`}>
            {isDark ? (
              <Sun className="w-4 h-4 text-amber-400 shrink-0 group-hover:rotate-45 transition-transform duration-300" />
            ) : (
              <Moon className="w-4 h-4 text-indigo-400 shrink-0 group-hover:-rotate-12 transition-transform duration-300" />
            )}
            {!isCollapsed && (
          ...
          {!isCollapsed && (
            <div
              className={`w-7 h-4 rounded-full p-0.5 transition-colors duration-300 flex items-center ${
                isDark ? 'bg-amber-500/80 justify-end' : 'bg-slate-700 justify-start'
              }`}
            >
              <span className="w-3 h-3 rounded-full bg-white shadow-sm transition-transform duration-300" />
            </div>
          )}
        </button>
      </div>
      ...
      <div className={`transition-all duration-300 ${isCollapsed ? 'p-1.5 pt-3' : 'p-3'} bg-slate-800/60 rounded-xl border border-slate-750/70 mt-2 space-y-2.5`}>
        {!isCollapsed ? (
          <div>
