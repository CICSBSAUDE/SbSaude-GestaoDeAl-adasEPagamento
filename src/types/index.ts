...
export interface User {
  ...
  name: string;
  email: string;
  googleEmail?: string; // E-mail da conta Google para login via SSO/Google
  cargo: string;
  area: string;
  centroCusto: string;
  centrosCusto?: string[]; // Multiple cost centers support
  phone?: string;
  roles: UserRole[];
...
export const isFinancialUser = (user?: User | null): boolean => {
  if (!user) return false;
  if (
    user.roles?.includes('ADMINISTRADOR') ||
    user.roles?.includes('FINANCEIRO') ||
    user.roles?.includes('TESOURARIA')
  ) {
    return true;
  }
  const area = (user.area || '').toLowerCase();
  const cargo = (user.cargo || '').toLowerCase();
  const cc = (user.centroCusto || '').toLowerCase();
  return (
    area.includes('finan') ||
    area.includes('contas a pagar') ||
    area.includes('tesouraria') ||
    area.includes('controladoria') ||
    cargo.includes('financeir') ||
    cargo.includes('finan') ||
    cargo.includes('tesouraria') ||
    cargo.includes('contas a pagar') ||
    cc.includes('3030') ||
    cc.includes('3000')
  );
};
...
export interface ProcessItem {
  ...
  cargoHabilitadoDocumento?: string; // Cargo / Centro de Custo Habilitado conforme POL-DIR-01 (Tabela Págs. 2-5)
  cargosHabilitadosSolicitante?: string[]; // Lista de cargos ou centros de custos autorizados a originar a solicitação
...
export interface RegraAlcada {
  ...
  cargoHabilitadoDocumento?: string; // Cargo / Centro de Custo Habilitado conforme POL-DIR-01 (Tabela Págs. 2-5)
  cargosHabilitadosSolicitante?: string[]; // Cargos / Centros de Custos que possuem legitimidade para originar a solicitação
  alcadasObrigatorias?: (1 | 2 | 3 | 4)[]; // Alçadas sequenciais obrigatórias da POL-DIR-01 (ex: [2], [2, 3, 4], [1, 2, 3, 4], [3, 4], [1, 4])
  cargosHabilitados1?: string[]; // Cargo 1ª alçada (N1 - Coordenação/Supervisão)
  cargosHabilitados2?: string[]; // Cargo 2ª alçada (N2 - Gerência)
  cargosHabilitados3?: string[]; // Cargo 3ª alçada (N3 - Diretoria de Operações)
  cargosHabilitados4?: string[]; // Cargo 4ª alçada (N4 - Diretoria Financeira / Executiva)
...
export interface EtapaAprovacao {
  ...
  cargoExigido: string;
  cargosHabilitados?: string[]; // Cargos Habilitados / Cadeia da Matriz de Alçadas
  areaExigida?: string;
  natureza?: string;
  aprovadorDesignadoId?: string;
  aprovadorDesignadoNome?: string;
  aprovadorDesignadoCargo?: string;
  aprovadorDesignadoArea?: string;
  aprovadorDesignadoCentroCusto?: string;
  semAprovadorCadastrado?: boolean;
...
export interface SystemNotification {
  ...
  solicitacaoId?: string;
  solicitacaoNumero?: string;
  targetTab?: string; // 'MY_QUEUE' | 'ALL_REQUESTS' | 'FINANCIAL' | 'TREASURY' | 'USERS' | 'MATRIX' | 'SGQ' | 'AUDIT' | 'DASHBOARD'
  targetAction?: 'OPEN_REQUEST' | 'NAVIGATE_TAB';
  lida: boolean;
  createdAt: string;
}
...
export interface AlertaAprovador {
  nivel: 1 | 2 | 3 | 4;
  nivelLabel: string;
  natureza: string;
  centroCusto: string;
...
export interface EnquadramentoResult {
  ...
  cadeiaAprovacao: EtapaAprovacao[];
  alertasAprovadores?: AlertaAprovador[];
  analiseFracionamento?: {
    possivelFracionamentoIdentificado: boolean;
    solicitacoesRelacionadasIds: string[];
    solicitacoesRelacionadasNumeros: string[];
