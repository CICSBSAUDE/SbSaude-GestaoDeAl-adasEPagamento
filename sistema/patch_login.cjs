...
// replace the useAuth destructuring
code = code.replace(/const { login, users, isLoading } = useAuth\(\);/, 'const { login, loginWithGoogle, users, isLoading } = useAuth();');
...
code = code.replace(/<\/button>\s*<\/form>/, '</button>\n' + buttonStr + '\n          </form>');
